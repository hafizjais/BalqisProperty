...
async function getShoplot(id: string) {
  try {
    return await fetchShoplot(id);
  } catch {
    return null;
  }
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const listing = await getShoplot(params.id);
...
export default async function ShoplotDetailPage({ params }: Props) {
  const listing = await getShoplot(params.id);
  if (!listing) notFound();

  const available = (listing.status || "available").toLowerCase() === "available";

  // Only show stat tiles that actually have a value in Airtable.
  const stats = [
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 pb-24 sm:px-6 lg:pb-8">
      <Breadcrumb
        items={[
          { label: "Home", href: "/" },
          { label: "Shop Lot", href: "/commercial/shop-lot" },
          { label: listing.title },
        ]}
      />
        ...
        <div>
          ...
          {listing.amenities.length > 0 && (
            <section className="mt-8">
              <h2 className="font-display text-2xl font-bold text-espresso">Amenities</h2>
